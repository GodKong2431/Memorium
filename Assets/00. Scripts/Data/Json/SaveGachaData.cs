...
public class SaveGachaData : ISaveData
{
    ...
    public void InitGachaData()
    {
        ...
        if (level == null)
        {
            ...
            pityCount = new List<int>();
        }

        if(level.Count<typeCount)
        {
        ...
        if (pityCount.Count < typeCount)
        {
            for (int i = pityCount.Count; i < typeCount; i++)
                pityCount.Add(0);
        }
    }
    ...
    public GachaLevelState GetGachaData(GachaType type)
    {
        ...
        return new GachaLevelState(type, level[index], drawCountInCurrentLevel[index], pityCount[index]);
    }

    public void SaveGachaLevel(GachaType type, int gachaLevel, int gachaDrawCount, int gachaPityCount)
    {
        int index = (int)type - (int)GachaType.Weapon;
        level[index] = gachaLevel;
        drawCountInCurrentLevel[index]= gachaDrawCount;
        pityCount[index] = Mathf.Max(0, gachaPityCount);
