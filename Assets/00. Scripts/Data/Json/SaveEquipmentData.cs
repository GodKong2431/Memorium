...
public class SaveEquipmentData
{
    ...
    //키 : 아이디     값 : 장비 데이터
    public Dictionary<int, EquipmentData> unlockEquipmentDict = new Dictionary<int, EquipmentData>();
    ...
    public void InitPlayerEquipmentData()
    {
        if (weaponId <= 0)
        {
            ...
            unLockEquipment = new List<EquipmentData>();
            unLockEquipment.Add(new EquipmentData(weaponId, 0));
            unLockEquipment.Add(new EquipmentData(helmetId, 0));
            unLockEquipment.Add(new EquipmentData(gloveId, 0));
            unLockEquipment.Add(new EquipmentData(armorId, 0));
            unLockEquipment.Add(new EquipmentData(bootsId, 0));
        }
    ...
    public void SetDict()
    {
        ...
        {
            //참고용 딕셔너리 제작
            unlockEquipmentDict[unLockEquipment[i].equipmentId] = unLockEquipment[i];
        }
    }
    ...
    public void SaveEquipment(InventoryItemContext context, BigDouble amount)
    {
        ...
        if (unlockEquipmentDict.ContainsKey(context.ItemId))
        {
...
public class SaveEquipmentData
{
    ...
    public void InitPlayerEquipmentData()
    {
        ...
        //강화 수치가 없으면 초기화 시켜라
        if (unLockEquipmentReinforcement == null)
        {
            unLockEquipmentReinforcement = new List<int>();
            for (int i = 0; i < unLockEquipmentKey.Count; i++)
            {
                unLockEquipmentReinforcement.Add(0);
            }
        }
        SetDict();
    }
