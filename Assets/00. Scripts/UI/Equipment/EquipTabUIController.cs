...
public class EquipTabUIController : UIControllerBase
{
    ...
    public EquipmentType TabType => tabType;
    ...
    public void SetReinforceController(EquipReinforceUIController controller)
    {
        reinforceController = controller;
    }
    ...
    {
        return root != null && tierPrefab != null && itemPrefab != null && starPrefab != null;
    }
    ...
    private void CreateItem(EquipTierUI tierUI, EquipListTable table, int tier, int orderInTier)
    {
        ...
        view.Render(
            IconManager.GetEquipmentIcon(table),
            GetLevelText(itemId),
            GetStarCount(tier),
    ...
    private void RefreshAllItems()
    {
        ...
        foreach (KeyValuePair<int, EquipItemView> pair in views)
        {
            ...
            view.RenderCount(ToCount(inventory.GetItemAmount(itemId)), mergeCount);
            view.SetDimmed(!equipModule.IsUnlocked(itemId));
            view.RenderLevel(GetLevelText(itemId));
        }
    }
    ...
    private string GetLevelText(int itemId)
    {
        ...
        EquipmentData equipmentData = equipModule.GetEquipment(itemId);
    ...
    {
        EquipReinforceUIController controller = GetReinforceController();
        if (controller == null || itemId == 0)
            return;

        controller.Show(itemId);
    }

    private EquipReinforceUIController GetReinforceController()
    {
        if (reinforceController != null)
            return reinforceController;
